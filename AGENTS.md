...
- `src/flow/action/`: one class per Flow card. `src/flow/jevAction.ts` holds the shared request and the invocation-local output tokens.
...
- `tests/`: mocked API and Flow contract tests. No live credentials required. `tests/setup.ts` stands in for the `homey` runtime module, which only exists on a Homey.
...
Use the `@basmilius/homey-common` base App, and register Flow cards as `FlowActionEntity` classes through `app.registry` rather than reaching for `homey.flow` directly. Keep HTTP validation independent of Homey for testing with an injected transport and clock. Use type aliases, four-space indentation and comments explaining constraints.
